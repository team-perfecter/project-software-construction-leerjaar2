...
async def admin_create_reservation(
    ...
    if not user:
        logging.info("Failed to create reservation: User ID %i does not exist", reservation.user_id, current_user.id)
        raise HTTPException(404, "User not found")
    
    # Check of vehicle bestaat.
    vehicle =  vehicle_model.get_one_vehicle(reservation.vehicle_id)
    if not vehicle:
        logging.info("Failed to create reservation: Vehicle ID %i does not exist", reservation.vehicle_id, current_user.id)
        raise HTTPException(404, "Vehicle not found")
    
    # Check of parking lot bestaat.
    parking_lot = parking_lot_model.get_parking_lot_by_lid(reservation.parking_lot_id)
    if not parking_lot:
        logging.info("Failed to create reservation: Parking Lot ID %i does not exist", reservation.parking_lot_id, current_user.id)
