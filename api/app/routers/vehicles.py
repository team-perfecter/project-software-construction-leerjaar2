...
from api.storage.vehicle_modal import vehicle_model
...
async def vehicles(vehicle_id: int):
    #Get user vehicle.
    vehicle = list(filter(lambda vehicle: vehicle["id"] == vehicle_id, vehicle_model.get_all_vehicles))[0]
...
async def vehicles_user(user_id: int):
    vehicles_user = list(filter(lambda vehicle: vehicle["user_id"] == user_id, vehicle_model.get_all_vehicles))
