...
async def vehicles():
    ...
    vehicles = vehicle_storage.get_all_vehicles() if auth["role"] == "ADMIN" else vehicle_storage.get_all_user_vehicles(temp_login_id)
...
async def vehicles_user(user_id: int):
    #Get user vehicles.
    vehicles_user = vehicle_storage.get_all_user_vehicles(user_id)
...
async def vehicle_create(vehicle: dict = Body(...)):
    #Create vehicle.
    updated_list = vehicle_storage.create_vehicle(vehicle)
    return updated_list
...
async def vehicle_delete(vehicle_id: int, current_user: User = Depends(get_current_user)):
    ...
    vehicle: Vehicle | None = vehicle_storage.get_one_vehicle(vehicle_id)
    ...
    vehicle_storage.delete_vehicle(vehicle.id)
