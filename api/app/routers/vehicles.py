...
async def vehicles_user(
    ...
    if not vehicles_user:
        ...
        logger.info("%s vehicles found for user %s", len(vehicles_user), user_id)
...
async def vehicle_update(vehicle_id: int, vehicle: dict = Body(...), user: User = Depends(get_current_user)):
    ...
    if vehicle_check["user_id"] == user.id:
        ...
        logger.warning("Vehicle %s does not belong to user %s. The vehicle was not updated", vehicle_id, user.id)
...
async def vehicle_delete(vehicle_id: int, user: User = Depends(get_current_user)):
    ...
    if vehicle is None:
        logging.warning(
            "A user with the ID of %s tried to delete a vehicle with the ID of %s, but the vehicle could not be found.",
            user.id, vehicle_id
        )
    ...
    if vehicle["user_id"] != user.id:
        logging.warning(
            "A user with the ID of %s tried to delete a vehicle with the ID of %s, but the vehicle does not belong to the user.",
            user.id, vehicle_id
        )
