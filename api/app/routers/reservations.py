...
async def create_reservation(
    reservation: ReservationCreate, current_user: User = Depends(get_current_user)
):
    ...
    if parking_lot == None:
        ...
        raise HTTPException(
            status_code=404, detail={"message": f"Parking lot does not exist"}
        )
    ...
    if vehicle == None:
        ...
        raise HTTPException(
            status_code=404, detail={"message": f"Vehicle does not exist"}
        )
    ...
    if reservation.start_time < datetime.now():
        raise HTTPException(
            status_code=400,
            detail={
                "message": f"invalid start time. The start time cannot be earlier than the current time. current time: {datetime.now()}, received time: {reservation.start_time}"
            },
        )
    ...
    if reservation.start_time >= reservation.end_time:
        raise HTTPException(
            status_code=400,
            detail={
                "message": f"invalid start time. The start time cannot be later than the end time. start time: {reservation.start_time}, end time: {reservation.end_time}"
            },
        )
    ...
    vehicle_reservations: list[Reservation] = (
        reservation_model.get_reservations_by_vehicle(vehicle["id"])
    )
    ...
    if conflicting_time:
        raise HTTPException(
            status_code=409,
            detail={
                "message": f"Requested time has an overlap with another reservation for this vehicle"
            },
        )
    ...
    if reservation.discount_code is not None:
        discount_code = discount_code_model.get_discount_code_by_code(
        ...
        if discount_code is None:
            ...
            raise HTTPException(
                status_code=404, detail={"message": "Discount code not found"}
            )
        use_discount_code_validation(
            discount_code, reservation, current_user, parking_lot
        )
    ...
    reservation_id = reservation_model.create_reservation(
        reservation, current_user.id, cost
    )
    # errorhandling etc.
...
async def delete_reservation(
    ...
    if reservation is None:
        logging.warning(
            "User with id %s tried to delete a reservation that does not exist: %s",
            current_user.id,
            reservation_id,
        )
        raise HTTPException(
    ...
    if reservation.user_id != current_user.id:
        logging.warning(
            "User with id %s tried to delete a reservation that does not belong to them: %s",
            current_user.id,
            reservation_id,
        )
        raise HTTPException(
    ...
    if not success:
        logging.error(
            "Failed to delete reservation with id %s for user %s",
            reservation_id,
            current_user.id,
        )
        raise HTTPException(
    ...
    logging.info(
        "User with id %s successfully deleted reservation with id %s",
        current_user.id,
        reservation_id,
    )
    raise HTTPException(
...
#     reservation_data: AdminReservationCreate,
#     current_user: User = Depends(require_role(UserRole.LOTADMIN, UserRole.SUPERADMIN)),
# ):
#     # Check of user bestaat
#     user = user_model.get_user_by_id(reservation_data.user_id)
#     if not user:
#         logger.info(
...
#     vehicle = vehicle_model.get_one_vehicle(reservation_data.vehicle_id)
#     if not vehicle:
#         logger.info(
...
#     parking_lot = parking_lot_model.get_parking_lot_by_lid(reservation_data.parking_lot_id)
#     if not parking_lot:
#         logger.info(
...
#     new_id = reservation_model.create_reservation(reservation_data, reservation_data.user_id, 0)

#     logger.info(
#         "Admin ID %i created reservation ID %i for user ID %i",
#         current_user.id,
#         new_id,
#         reservation_data.user_id,
#     )
...
#     return JSONResponse(
#         content={"message": "Reservation created", "reservation_id": new_id},
#         status_code=201,
#     )
