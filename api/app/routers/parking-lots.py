...
async def get_parking_lot_by_id(location: str, current_user: User = Depends(get_current_user)):
    parking_lots: list[Parking_lot] = storage.get_parking_lot_by_location(location)
    logging.warning("A user with the ID of %i tried to retrieve parking lots in the location %s.", current_user.id, location)
    return parking_lots