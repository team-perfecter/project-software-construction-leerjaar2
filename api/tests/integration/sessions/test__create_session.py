...
def test_start_session_vehicle_not_owned(client_with_token):
    ...
    create_response = superadmin_client.post("/vehicles/create", json=vehicle, headers=superadmin_headers)
    ...
    assert "Vehicle does not belong to current user" in response.json()["detail"]["message"]
...
def test_stop_session_from_reservation_forbidden(client_with_token):
    ...
    reservation_response = client.post("/reservations/create", json=reservation_data, headers=headers)
    ...
    assert response.status_code == 403
    assert "Cannot stop a session that was started from a reservation" in response.json()["detail"]
...
def test_start_session_from_reservation_not_owned(client_with_token):
    ...
    reservation_response = superadmin_client.post("/reservations/create", json=reservation_data, headers=superadmin_headers)
    reservation_id = reservation_response.json()["reservation"]["id"]
    
    # Try to start session as different user
    user_client, user_headers = client_with_token("extrauser")
    response = user_client.post(f"/sessions/reservations/{reservation_id}/start", headers=user_headers)
