...
from api.tests.conftest import setup_reservation_prerequisites, create_test_vehicle, get_last_pid
...
def test_create_reservation_missing_field(client_with_token):
    ...
    response = client.post("/reservations/create", json=incomplete_data, headers=headers)
...
def test_create_reservation_conflicting_time(client_with_token):
    ...
    first_response = client.post("/reservations/create", json=first_reservation, headers=headers)
    ...
    if first_response.status_code == 201:
        ...
        second_response = client.post("/reservations/create", json=second_reservation, headers=headers)
...
def test_create_multiple_non_overlapping_reservations(client_with_token):
    ...
    first_response = client.post("/reservations/create", json=first_data, headers=headers)
    ...
    second_response = client.post("/reservations/create", json=second_data, headers=headers)
...
def test_create_reservation_different_roles(client_with_token, role):
    ...
    client, headers, vehicle_id, parking_lot_id = setup_reservation_prerequisites(
        client_with_token,
        role=role,
        vehicle_plate=f"ROLE-{role.upper()}"
    )
...
@patch("api.app.routers.reservations.parkingLot_model.get_parking_lot_by_lid", return_value=None)
...
@patch("api.models.reservation_model.Reservation_model.create_reservation", side_effect=Exception("Database error"))
def test_create_reservation_server_error_mock(mock_create, client_with_token):
    """Test: Mock database error during creation"""
    client, headers, vehicle_id, parking_lot_id = setup_reservation_prerequisites(
        client_with_token,
        vehicle_plate="MOCK-ERROR"
    )
