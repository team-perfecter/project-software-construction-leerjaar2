...
def test_start_session_authorized(client_with_token):
    ...
    response = client_instance.post(
        "/parking-lots/1/sessions/start", headers=headers, json=vehicle
    )
...
def test_start_session_nonexistent_parking_lot(client_with_token):
    ...
    vehicle = {"vehicle_id": 1}
    response = client_instance.post(
        "/parking-lots/999/sessions/start", headers=headers, json=vehicle
    )
...
def test_start_session_lid_not_int(client_with_token):
    ...
    vehicle = {"vehicle_id": 1}
    response = client_instance.post(
        "/parking-lots/hallo/sessions/start", headers=headers, json=vehicle
    )
    assert response.status_code == 422
...
def test_stop_session_authorized(client_with_token):
    ...
    response = client_instance.post(
        "/parking-lots/1/sessions/stop", headers=headers, json=session
    )
    assert response.status_code == 200
...
def test_stop_session_from_nonexistent_parking_lot(client_with_token):
    ...
    session = {"session_id": 1}
    response = client_instance.post(
        "/parking-lots/999/sessions/stop", headers=headers, json=session
    )
    assert response.status_code == 404
...
def test_stop_session_lid_not_int(client_with_token):
    ...
    session = {"session_id": 1}
    response = client_instance.post(
        "/parking-lots/hallo/sessions/stop", headers=headers, json=session
    )
    assert response.status_code == 422
